...
| **Similarity (Dice Similarity Coefficient, DSC)** | ![DSC公式](https://latex.codecogs.com/svg.latex?\Large&space;\text{DSC}=\frac{2|X\cap{Y}|}{|X|+|Y|}) | Measures the overlap between detected events ($A$) and ground truth events ($B$). |
