...
| **Similarity** |$`\text{D(A,B)} = \dfrac{{2|A \cap B|}{|A| + |B|}`$ | Measures overlap between detected events and ground truth events.     |
...
| **Error Rate**            | $`Error\ Rate = \left| \overline{\mathrm{scRNA}\psi} - \overline{\mathrm{bulk}\psi} \right| \times 100`$  | Measures the relative deviation of predictions from ground truth.               |
